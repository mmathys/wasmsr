...
func getIndent(ctx context.Context, len int) string {
	noIndent := ctx.Value("no-indent").(bool)
	if !noIndent {
...
func makeSnapshot(ctx context.Context, fsContext *sys.FSContext, ce *callEngine, moduleInst *wasm.ModuleInstance, op *interpreterOp) {
	//indent := getIndent(ctx, len(ce.frames))
	//fmt.Printf("%s! next: %v, ", indent, op.kind.String())

	// print trace information
	timeStart := time.Now()

	var frames []wasm.CallFrame
	...
	fmt.Printf(", stack: %v", ce.stack)
	fmt.Printf(", globals: %v", moduleInst.Globals)
	fmt.Println()
	...
	for _, global := range moduleInst.Globals {
	...
	for _, frame := range ce.frames {
		framePb := &proto.Frame{
			Pc:            frame.pc,
			FunctionIndex: frame.f.source.Idx,
		}
	...
	*snapshot = proto.Snapshot{
		...
		Frames:      framesPb,
		Memory:      memoryPb,
		LastFd:      fsContext.GetLastFD(),
		OpenedFiles: fsContext.GetOpenedFiles(),
	}

	timeStartExport := time.Now()

	if ctx.Value("export").(bool) {
		exportSnapshot(ctx, snapshot)
		fmt.Println("Exported snapshot.")
	}

	if ctx.Value("benchmark").(bool) {
		// serialize
		now := time.Now()
		diff1 := timeStartExport.Sub(timeStart)
		diff2 := now.Sub(timeStartExport)
...
func exportSnapshot(ctx context.Context, snapshot *proto.Snapshot) {
	// write to disk
	out, err := pb.Marshal(snapshot)
	if err != nil {
...
func applySnapshot(snapshot *proto.Snapshot, fsContext *sys.FSContext, e *moduleEngine, ce *callEngine, moduleInst *wasm.ModuleInstance) {
	// Stack
	ce.stack = snapshot.GetStack()

	// Globals
	moduleInst.Globals = nil
	for _, global := range snapshot.GetGlobals() {
		...
		moduleInst.Globals = append(moduleInst.Globals, globalInstance)
	}

	// Frames
	ce.frames = nil
	for _, frame := range snapshot.GetFrames() {
		f := e.functions[frame.GetFunctionIndex()]
		ce.pushFrame(&callFrame{f: f, pc: frame.Pc})
	}

	// Memory
	moduleInst.Memory = &wasm.MemoryInstance{
		Buffer: snapshot.GetMemory().GetBuffer(),
		Min:    snapshot.GetMemory().GetMin(),
		Max:    snapshot.GetMemory().GetMax(),
		Cap:    snapshot.GetMemory().GetCap(),
	}

	// File System
	fsContext.SetLastFD(snapshot.GetLastFd())
	fsContext.SetOpenedFiles(snapshot.GetOpenedFiles())
}
...
func (e *moduleEngine) Call(ctx context.Context, m *wasm.CallContext, f *wasm.FunctionInstance, params ...uint64) (results []uint64, err error) {
	...
	ce.callFunction(ctx, m, compiled, true, false)

	results = wasm.PopValues(f.Type.ResultNumInUint64, ce.popValue)
	return
}

func (e *moduleEngine) Resume(ctx context.Context, m *wasm.CallContext, f *wasm.FunctionInstance, snapshot *proto.Snapshot) (results []uint64, err error) { // Note: The input parameters are pre-validated, so a compiled function is only absent on close. Updates to
	// code on close aren't locked, neither is this read.
	compiled := e.functions[f.Idx]
	if compiled == nil { // Lazy check the cause as it could be because the module was already closed.
		if err = m.FailIfClosed(); err == nil {
	...
	*/
	...
	for len(ce.frames) > 0 {
		...
		ce.callFunction(ctx, m, curFrame.f, false, resume)
		resume = false

		// if has next, advance ptr.
		if len(ce.frames) > 0 {
...
func (ce *callEngine) callFunction(ctx context.Context, callCtx *wasm.CallContext, f *function, createCallFrame bool, resume bool) {
	indent := strings.Repeat("  ", len(ce.frames))
	if f.hostFn != nil {
		if !ctx.Value("quiet").(bool) {
			fmt.Printf("%sEnter %s (Go)\n", indent, f.source.Definition().Name())
		}
		ce.callGoFuncWithStack(ctx, callCtx, f)
	} else if lsn := f.source.FunctionListener; lsn != nil {
		if !ctx.Value("quiet").(bool) {
			fmt.Println("Enter Wasm function: (TODO)")
		}
		ce.callNativeFuncWithListener(ctx, callCtx, f, lsn)
	} else {
		if !ctx.Value("quiet").(bool) {
			fmt.Printf("%sEnter %s (native)\n", indent, f.source.Definition().Name())
		}
		ce.callNativeFunc(ctx, callCtx, f, createCallFrame, resume)
	}
}
...
func (ce *callEngine) callNativeFunc(ctx context.Context, callCtx *wasm.CallContext, f *function, createCallFrame bool, resume bool) {
	...
	for frame.pc < bodyLen {
		...
		if ctx.Value("snapshot") != nil && !resume && (atomic.CompareAndSwapUint32(&wasm.BreakpointFlag, 1, 0) || ctx.Value("all") == true) {
			// snapshot *before* execution.
			makeSnapshot(ctx, fsContext, ce, moduleInst, op)

			// trap if we want.
			if ctx.Value("trap").(bool) {
		...
		case wazeroir.OperationKindCall:
			ce.callFunction(ctx, callCtx, functions[op.us[0]], true, false)
			frame.pc++
		case wazeroir.OperationKindCallIndirect:
			...
			ce.callFunction(ctx, callCtx, tf, true, false)
			frame.pc++
...
func (ce *callEngine) callNativeFuncWithListener(ctx context.Context, callCtx *wasm.CallContext, f *function, fnl experimental.FunctionListener) context.Context {
	...
	ce.callNativeFunc(ctx, callCtx, f, true, false)
	// TODO: This doesn't get the error due to use of panic to propagate them.
