...
func (f *importedFn) Resume(ctx context.Context, snapshot *proto.Snapshot) (ret []uint64, err error) {
...
// Call implements the same method as documented on api.Function.
func (f *FunctionInstance) Resume(ctx context.Context, snapshot *proto.Snapshot) (ret []uint64, err error) {
	if ctx == nil {
		ctx = context.Background()
	}
	mod := f.Module
