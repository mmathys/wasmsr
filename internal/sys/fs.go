...
func (c *FSContext) GetOpenedFiles() map[uint32]*proto.FileEntry {
	newMap := make(map[uint32]*proto.FileEntry)
	for fd, entry := range c.openedFiles {
		newMap[fd] = &proto.FileEntry{
			Path: entry.Path,
		}
		isDir := false

		if entry.File != nil {
			fileInfo, err := entry.File.Stat()
			if err != nil {
				log.Panicln(err)
			}
		...
		newMap[fd].IsDir = isDir || entry.Path == "/"

		newMap[fd].Offset = 0
		if entry.File != nil {
			offset, err := entry.File.(io.Seeker).Seek(0, io.SeekCurrent)
			if err != nil {
				log.Panicln("error reading seek pos", err)
			}
			newMap[fd].Offset = offset
		}
	}
	return newMap
}
...
func (c *FSContext) SetOpenedFiles(openedFiles map[uint32]*proto.FileEntry) {
	for fd, fileEntry := range openedFiles {
		c.openedFiles[fd] = &FileEntry{
			Path: fileEntry.Path,
		}
		...
		// if file is not a directory, open the file.
		file, err := os.OpenFile(fileEntry.Path, os.O_APPEND|os.O_CREATE|os.O_RDWR, 0644)
		if err != nil {
			log.Panicln("error opening file in SetOpenedFiles", err)
		}

		c.openedFiles[fd].File = file

		// seek
		newOffset, err := c.openedFiles[fd].File.(io.Seeker).Seek(fileEntry.Offset, 0)
		if err != nil {
			log.Panicln("error seeking file", err)
		} else if newOffset != fileEntry.Offset {
			log.Panicln("offset did not match")
		}
	}
}
